...
int main(int argc, const char *argv[]) {
  ...
  if(argc < 3) {fprintf(stderr, "Usage: ./grep \"[pattern]\" [file path]\n "); exit(1);}
  char exp_buf[BLKSIZE];
  argv++; strcpy(exp_buf, *argv);
  argv++; strcpy(savedfile, *argv);
	zero = (unsigned *)malloc(nlall*sizeof(unsigned));
	tfname = mkdtemp(tmpXXXXX);
	init(); 
  search_file(savedfile,exp_buf);
  quit(0);
 return 0;
}
...
unsigned int* address(void) {  int sign;  unsigned int *a, *b;  int opcnt, nextopand;  int c;
  ...
  do {
    ...
    if ('0'<=c && c<='9') {  peekc = c;  if (!opcnt)  { a = zero; }  a += sign*getnum();
      ...
      case '\'':
        c = getchar();  if (opcnt || c<'a' || 'z'<c) { error(Q); }  a = zero;
...
void search_file( char* file,  char* searchfor) {
  readfile(file);
  search(searchfor);
}
